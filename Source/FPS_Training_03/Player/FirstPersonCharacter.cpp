...
void AFirstPersonCharacter::BeginPlay()
{
	...
	Gun = GetWorld()->SpawnActor<AGun_Weapon>(GunBlueprint);
	//Attach gun mesh component to Skeleton, doing it here because the skeleton is not yet created in the constructor
	Gun->AttachToComponent(Mesh1P, FAttachmentTransformRules(EAttachmentRule::SnapToTarget, true), TEXT("GripPoint"));
	Gun->AnimInstance1P = Mesh1P->GetAnimInstance();
	// Bind fire event
	InputComponent->BindAction("Fire", IE_Pressed, Gun, &AGun_Weapon::OnFire);
...
//void FirstPersonCharacter::TouchUpdate(const ETouchIndex::Type FingerIndex, const FVector Location)
//{
...
bool AFirstPersonCharacter::EnableTouchscreenMovement(class UInputComponent* PlayerInputComponent)
{
	if (FPlatformMisc::SupportsTouchInput() || GetDefault<UInputSettings>()->bUseMouseForTouch)
	{
		...
		//PlayerInputComponent->BindTouch(EInputEvent::IE_Repeat, this, &FirstPersonCharacter::TouchUpdate);
		return true;
	}
